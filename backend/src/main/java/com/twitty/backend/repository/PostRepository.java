...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(nativeQuery = true, value =  "SELECT p.id as postId, p.created_at as createdAt, p.created_by as createdByUserId, p.image as postImage, " +
            "        p.is_hash_tagged_post as isHashTagged, p.type as postType, p.updated_at as postUpdatedAt,  " +
...
SELECT p.id as postId, p.created_at as createdAt, p.created_by as createdByUserId, p.image as postImage,
		p.is_hash_tagged_post as isHashTagged, p.type as postType, p.updated_at as postUpdatedAt,
        p.text as postText, u.username, ui.profile_picture as userProfilePicture
	FROM post p
    join user u on u.id = p.created_by
    join user_information ui on ui.id=u.user_information_id
    order by p.updated_at desc limit 0,7;
