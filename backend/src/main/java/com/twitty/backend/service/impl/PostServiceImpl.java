...
import com.twitty.backend.entity.*;
...
public class PostServiceImpl implements PostService {
    ...
    public List<PostResponseDTO> getPosts(int limit, int offset) {
        ...
        for(PostDTO pd: postDTOs){
            ...
            postResponseDTO.setNoOfComments(pd.getnoOfComments());
    ...
    @Override
    public CommentResponse saveComment(String text, long fromPostId, long fromCommentId, MultipartFile image) {
        ...
        Optional<Post> optionalPost = postRepo.findById(fromPostId);
        ...
        commentResponse.setCommentCreatedAt(comment.getCreatedAt());
        commentResponse.setCreatedByUserId(comment.getCreatedBy());
        ...
        commentResponse.setCommentUpdatedAt(DateUtil.convertTimeDiff(comment.getUpdatedAt()));
        commentResponse.setUsername(user.getUsername());
        commentResponse.setUserTag(ui.getUserTag());
        if(comment.getImage() != null && !comment.getImage().equalsIgnoreCase("")){
            commentResponse.setCommentImage("api/user/image/post/".concat(comment.getImage()));
        }
        if(ui.getProfilePicture() != null && !ui.getProfilePicture().equalsIgnoreCase("")){
            commentResponse.setUserProfilePicture("api/user/image/profile/".concat(ui.getProfilePicture()));
        }
    ...
    public ResponseEntity<?> getCommentDetails(Long postId) {
        ...
        if(optionalPostDTO.isPresent()){
            ...
            postResponseDTO.setNoOfComments(postDTO.getnoOfComments());
